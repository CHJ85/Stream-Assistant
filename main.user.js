...
// @version      3.2.3
// @description  Adds keyboard shortcuts, filters, EQ controls (Bass/Vocals), Censor bleep, zoom controls, Mono Downmix, visualizers, A-B Looping, auto-splitting Canvas recorder, and raw/effects video recording.
// @author       CHJ85
// @match        https://*.max.com/*
...
(function() {
    ...
    let recordCtx = null;
    let canvasSplitTimer = null;
    let recordingPart = 1;
    let isAutoSplitting = false;
    const AUTO_SPLIT_MS = 14 * 60 * 1000; // Auto-split every 14 minutes to prevent browser RAM limits
    ...
    // Canvas/Effects Recording with Auto-Split Memory Protection
    function toggleCanvasRecording() {
        if (!video) loadVideo();
        if (!video) return;

        // Stop recording manually if active
        ...
        const scheduleAutoSplit = () => {
            clearTimeout(canvasSplitTimer);
            canvasSplitTimer = setTimeout(() => {
                if (isCanvasRecording && canvasMediaRecorder && canvasMediaRecorder.state === 'recording') {
        ...
        const drawFrame = () => {
            ...
            if (!video || !document.body.contains(video)) {
                console.warn("Stream Assistant: Video element removed. Halting recording.");
    ...
    function initAudioGraph() {
        ...
        video.dataset.audioNodeBound = 'true';
