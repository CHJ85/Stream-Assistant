...
// @version      3.2.2
// @description  Adds keyboard shortcuts, filters, EQ controls (Bass/Vocals), Censor bleep, zoom controls, Mono Downmix, visualizers, A-B Looping, and raw/effects video recording (with auto-pause).
// @author       CHJ85
// @match        https://*.max.com/*
...
(function() {
 ...
 matrixDrops: Array(100).fill(0)
    ...
    function toggleABLoop() {
        ...
        if (!isLoopActive && loopStartTime === null) {
            // Step 1: Set Loop Start Point
            loopStartTime = currentTime;
            console.log(`Stream Assistant: A-B Loop Start set at ${loopStartTime.toFixed(2)}s`);
        } else if (!isLoopActive && loopStartTime !== null) {
            // Step 2: Set Loop End Point and Start Looping
            if (currentTime <= loopStartTime) {
                console.warn("Stream Assistant: Loop End must be set after Loop Start.");
                return;
            }
            loopEndTime = currentTime;
            isLoopActive = true;
            console.log(`Stream Assistant: A-B Loop Active from ${loopStartTime.toFixed(2)}s to ${loopEndTime.toFixed(2)}s`);
        } else {
            // Step 3: Cancel Loop & Reset
            resetABLoop();
            console.log("Stream Assistant: A-B Loop Cancelled.");
        }
    }
    ...
    function setupABLoopListener() {
        if (!video || video.dataset.abLoopBound === "true") return;
        
        video.addEventListener('timeupdate', () => {
            if (isLoopActive && loopStartTime !== null && loopEndTime !== null) {
                if (video.currentTime >= loopEndTime || video.currentTime < loopStartTime) {
                    video.currentTime = loopStartTime;
                }
            }
        });
        
        video.dataset.abLoopBound = "true";
    }
 ...
 'video/mp4'
    ...
    function toggleCanvasRecording() {
        ...
        canvasMediaRecorder.ondataavailable = (e) => {
            ...
            const drawFrame = () => {
                ...
                // If the video is destroyed (e.g., episode change), stop recording to save the file and kill the loop.
                if (!video || !document.body.contains(video)) {
                    console.warn("Stream Assistant: Video element removed. Halting recording to prevent memory leak.");
                ...
                // Overlay Visualizer if active (Drawn above video, unaffected by video filters)
                ...
                spacebarTimer = setTimeout(() => {
                    if (spacebarHeldDown) {
                        ...
                        if (video) {
                            ...
                            // Force video to play if it was paused when hold started
                            if (video.paused) {
    ...
    function adjustPlaybackSpeed(direction) {
        if (video) {
            ...
            const currentRate = video.playbackRate || playbackSpeed;
            playbackSpeed = clamp(currentRate + direction * config.playbackSpeedStep, 0.25, 4);
    ...
    function toggleAspectRatio() {
        ...
        video.style.setProperty('object-fit', options[aspectRatioOption].fit, 'important');
        video.style.setProperty('object-position', options[aspectRatioOption].pos, 'important');
    ...
    function initAudioGraph() {
        ...
        // Check if we've already bound an audio source to this specific HTML node
        if (video.dataset.audioNodeBound === 'true' && audioContextData) {
            return;
        }
        const source = context.createMediaElementSource(video);
        video.dataset.audioNodeBound = 'true'; // Mark it as permanently bound
 ...
 streamDestination
    ...
    function setVisualizer(mode) {
        ...
        if (currentVisMode !== 0 && !animationFrameId) {
            renderVisualizer();
        }
    }
    
    function renderVisualizer() {
        // ADDED: Check if video is detached from the DOM (!document.body.contains(video))
        if (!visCtx || !visCanvas || !audioContextData || currentVisMode === 0 || !video || !document.body.contains(video)) {
                    ...
                    mutation.addedNodes.forEach(node => {
                        if (node.tagName === 'VIDEO') {
                            if (node.src && blockedHosts.some(host => node.src.includes(host))) { 
                                node.pause(); 
                                node.remove(); 
                                // Only nullify if the blocked ad hijacked our global video reference
